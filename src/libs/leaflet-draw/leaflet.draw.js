...
!(function (t, e, i) {
  ...
  L.Renderer.include({
    _updateMayBienAp: function (t) {
      const e = t._xuLy(t._getLatLngI1(), t._getRadiusR1()),
        i = e[0],
        o = Math.max(Math.round(e[1]), 1),
        n = Math.max(Math.round(e[2]), 1) || o,
        s = t._xuLy(t._getLatLngI2(), t._getRadiusR2()),
        a = s[0],
        r = Math.max(Math.round(s[1]), 1),
        h = Math.max(Math.round(s[2]), 1) || r,
        l = "a" + o + "," + n + " 0 1,0 ",
        c = "a" + r + "," + h + " 0 1,0 ",
        d = t._xuLy(t._getLatLngA(), t._getRadiusR1())[0],
        _ = t._xuLy(t._getLatLngN(), t._getRadiusR1())[0],
        u = t._xuLy(t._getLatLngB(), t._getRadiusR1())[0],
        p =
          "M" +
          d.x +
          " " +
    ...
    (L.MayBienAp = L.Path.extend({
      options: { fill: !1, chieuDai: 150, gocXoay: 0 },
      ...
      _getRadiusR1: function () {
        return (
          (4 * this.options.chieuDai) / (2 + 3 * Math.sin(Math.acos(1 / 3)) * 4)
        );
      },
      ...
      _getLatLngM: function () {
        const t = L.GeometryUtil.angle(
            this._map,
            this._getLatLngA(),
            this._getLatLngB()
          ),
          e = this._getLatLngA().distanceTo(this._getLatLngB());
        return L.GeometryUtil.destination(this._getLatLngA(), t, e / 2);
      },
      _getLatLngN: function () {
        const t = this._getLatLngM(),
          e = this._getLatLngI1(),
          i = L.GeometryUtil.angle(this._map, t, this._getLatLngB()),
          o = L.GeometryUtil.angle(this._map, t, e),
          n = o - i,
          s = t.distanceTo(e),
          a = Math.sin((n * Math.PI) / 180) * s;
        return L.GeometryUtil.destination(
          this._getLatLngM(),
          i - 90,
          (3 * a) / 4
      ...
      _xuLy: function (t, e) {
        var i,
          ...
          r = this._map,
          h = r.options.crs;
        if (h.distance === L.CRS.Earth.distance) {
          var l = Math.PI / 180,
            c = e / L.CRS.Earth.R / l,
            d = r.project([a + c, s]),
            _ = r.project([a - c, s]),
            u = d.add(_).divideBy(2),
            p = r.unproject(u).lat,
            f =
              Math.acos(
                (Math.cos(c * l) - Math.sin(a * l) * Math.sin(p * l)) /
                  (Math.cos(a * l) * Math.cos(p * l))
              ) / l;
          (isNaN(f) || 0 === f) && (f = c / Math.cos((Math.PI / 180) * a)),
            (i = u.subtract(r.getPixelOrigin())),
            (o = isNaN(f) ? 0 : u.x - r.project([p, s - f]).x),
            (n = u.y - d.y);
        } else {
          var g = h.unproject(h.project(t).subtract([e, 0]));
          (i = r.latLngToLayerPoint(t)),
            (o = i.x - r.latLngToLayerPoint(g).x),
            (n = e);
        }
        return [i, o, n];
      },
      ...
      _updateBounds: function () {
        ...
        this._pxBounds = L.bounds(
          this._map.latLngToLayerPoint(t._northEast),
          this._map.latLngToLayerPoint(t._southWest)
        );
      },
      _update: function () {
      ...
      rotate: function (t) {
        const e = this._map,
          i = this._latlng,
          o = L.GeometryUtil.angle(e, i, t);
        (this.options.gocXoay = o), this.redraw();
      },
      resize: function (t) {
        const e = this._map,
          i = this._getLatLngC(),
          o = this._latlng,
          n = L.GeometryUtil.angle(e, o, i),
          s = L.GeometryUtil.angle(e, o, t),
          a = Math.abs(s - n),
          r = Math.cos((a * Math.PI) / 180) * o.distanceTo(t);
        (this.options.chieuDai = Math.abs(2 * r)), this.redraw();
      },
      move: function (t) {
        (this._latlng = t), this.redraw();
      },
    })),
    (L.mayBienAp = function (t, e) {
      return new L.MayBienAp(t, e);
    }),
    L.Layer.include({
      getCenterCus: function () {
        const t = this._map.project(
            this._bounds._northEast,
            this._map.getZoom()
          ),
          e = this._map.project(this._bounds._southWest, this._map.getZoom());
        return this._map.unproject(
          L.point((t.x + e.x) / 2, (t.y + e.y) / 2),
          this._map.getZoom()
        );
      },
    }),
    (L.ThanhCai = L.Polyline.extend({
      options: { chieuDai: 300, gocXoay: 0, distanceRotateMarker: 20 },
      initialize: function (t, e) {
        L.setOptions(this, e);
        const i = this._thanhCaiLatLngs(t);
        L.Polyline.prototype.initialize.call(this, i, e);
      },
      _thanhCaiLatLngs: function (t) {
        const e = this.options.gocXoay,
          i = e + 180;
        return [
          L.GeometryUtil.destination(t, e, this.options.chieuDai / 2),
          L.GeometryUtil.destination(t, i, this.options.chieuDai / 2),
        ];
      },
      ...
      rotate: function (t) {
        const e = this._map,
          i = this.getCenterCus(),
          o = L.GeometryUtil.angle(e, i, t);
        (this.options.gocXoay = o + 90),
          this.setLatLngs(this._thanhCaiLatLngs(i));
      },
      resize: function (t) {
        const e = this._map,
          i = this.getLatLngs(),
          o = this.getCenterCus(),
          n = i[0],
          s = L.GeometryUtil.angle(e, o, n),
          a = L.GeometryUtil.angle(e, o, t),
          r = Math.abs(a - s),
          h = Math.cos((r * Math.PI) / 180) * o.distanceTo(t);
        (this.options.chieuDai = Math.abs(2 * h)),
          this.setLatLngs(this._thanhCaiLatLngs(o));
      },
      move: function (t) {
        this.setLatLngs(this._thanhCaiLatLngs(t));
      },
    })),
    (L.thanhCai = function (t, e, i) {
      return new L.ThanhCai(t, e, i);
    }),
    (L.Role = L.Polyline.extend({
      options: { chieuDai: 50, chieuRong: 25, gocXoay: 0 },
      initialize: function (t, e) {
        L.setOptions(this, e);
        const i = this._roleLatLngs(t);
        L.Polyline.prototype.initialize.call(this, i, e);
      },
      _roleLatLngs: function (t) {
        const e = -this.options.gocXoay,
          i = -90 - this.options.gocXoay,
          o = i + 180,
          n = 180 + e,
          s = 180 + e,
          a = L.GeometryUtil.destination(t, -e, this.options.chieuRong / 2),
          r = L.GeometryUtil.destination(a, -i, this.options.chieuDai / 2),
          h = L.GeometryUtil.destination(a, -o, this.options.chieuDai / 2);
        return [
          r,
          h,
          L.GeometryUtil.destination(h, -n, this.options.chieuRong),
          L.GeometryUtil.destination(r, -s, this.options.chieuRong),
          r,
        ];
      },
      getRotateMarker: function () {
      ...
      move: function (t) {
        this.setLatLngs(this._roleLatLngs(t));
      },
      resize: function (t) {
        const e = this.getLatLngs(),
          ...
          s = e[1],
          a = e[3],
          r = L.GeometryUtil.closestOnSegment(o, i, n, s),
          h = L.GeometryUtil.closestOnSegment(o, i, n, a),
          l = L.GeometryUtil.angle(o, r, n),
          c = L.GeometryUtil.angle(o, h, n),
          d = L.GeometryUtil.angle(o, r, t),
          _ = L.GeometryUtil.angle(o, h, t),
          u = Math.abs(d - l),
          p = Math.abs(_ - c),
          f = Math.cos((u * Math.PI) / 180) * r.distanceTo(t),
          g = Math.cos((p * Math.PI) / 180) * h.distanceTo(t);
        (this.options.chieuDai = Math.abs(this.options.chieuDai / 2 + f)),
          (this.options.chieuRong = Math.abs(this.options.chieuRong / 2 + g)),
          this.setLatLngs(this._roleLatLngs(i));
      },
      rotate: function (t) {
        const e = this._map,
          i = this.getCenterCus(),
          o = L.GeometryUtil.angle(e, i, t);
        (this.options.gocXoay = o), this.setLatLngs(this._roleLatLngs(i));
      },
      getLatLngSnaps: function () {
        const t = this.getLatLngs(),
          e = t[0],
          i = t[1];
        return [
          L.GeometryUtil.destination(
    ...
    (L.drawLocal = {
      draw: {
        ...
        handlers: {
          ...
          role: { tooltip: { start: "Click để vẽ rơ-le." } },
          thanhCai: { tooltip: { start: "Click để vẽ thanh cái" } },
          mayBienAp: { tooltip: { start: "Click để vẽ máy biến áp" } },
          duongDay: {
            error: "<strong>Error:</strong> shape edges cannot cross!",
            tooltip: {
              start: "Click to start drawing line.",
    ...
    (L.Draw.DuongDay = L.Draw.Feature.extend({
      ...
      initialize: function (t, e) {
        L.Browser.touch && (this.options.icon = this.options.touchIcon),
          (this.options.drawError.message =
            L.drawLocal.draw.handlers.duongDay.error),
          e &&
            e.drawError &&
            (e.drawError = L.Util.extend(
              {},
      ...
      addVertex: function (t) {
        if (
          ...
          return void this._showErrorTooltip();
        this._errorShown && this._hideErrorTooltip();
        const e = this._poly.getLatLngs();
        if (0 !== e.length) {
          const i = e[e.length - 1],
            o = this._map.latLngToLayerPoint(i),
            n = this._map.latLngToLayerPoint(t),
            s = L.point(o.x, n.y),
            a = this._map.layerPointToLatLng(s);
          this._markers.push(this._createMarker(a)), this._poly.addLatLng(a);
        }
        this._markers.push(this._createMarker(t)),
          this._poly.addLatLng(t),
          3 === this._poly.getLatLngs().length &&
            this._map.addLayer(this._poly),
          this._vertexChanged(t, !0);
      },
      completeShape: function () {
      ...
      _drawGuide: function (t, e) {
        var i = L.point(t.x, e.y);
        this._drawGuidePath(t, i), this._drawGuidePath(i, e);
      },
      _updateGuideColor: function (t) {
        if (this._guidesContainer)
          for (
    ...
    (L.Draw.ThanhCai = L.Draw.Feature.extend({
      ...
      initialize: function (t, e) {
        (this.type = L.Draw.ThanhCai.TYPE),
          (this._initialLabelText =
            L.drawLocal.draw.handlers.thanhCai.tooltip.start),
          L.Draw.Feature.prototype.initialize.call(this, t, e);
      },
      addHooks: function () {
        L.Draw.Feature.prototype.addHooks.call(this),
        ...
        this._tooltip.updatePosition(e),
          ...
          this._marker
            ? ((e = this._mouseMarker.getLatLng()),
              this._map.removeLayer(this._marker),
              (this._marker = this._createThanhCai(e)),
      ...
      _createThanhCai: function (t) {
        return L.thanhCai(t, this.options);
      },
      _onClick: function () {
        this._fireCreatedEvent(),
          this.disable(),
      ...
      _fireCreatedEvent: function () {
        var t = this._createThanhCai(this._mouseMarker.getLatLng());
        L.Draw.Feature.prototype._fireCreatedEvent.call(this, t);
      },
    })),
    (L.Draw.Role = L.Draw.Feature.extend({
      ...
      initialize: function (t, e) {
        (this.type = L.Draw.Role.TYPE),
          (this._initialLabelText =
            L.drawLocal.draw.handlers.role.tooltip.start),
          L.Draw.Feature.prototype.initialize.call(this, t, e);
      },
      addHooks: function () {
        L.Draw.Feature.prototype.addHooks.call(this),
      ...
      _onMouseMove: function (t) {
        ...
        this._tooltip.updatePosition(e),
          ...
          this._marker
            ? ((e = this._mouseMarker.getLatLng()),
              this._map.removeLayer(this._marker),
              (this._marker = this._createRole(e)),
              this._map.addLayer(this._marker))
            : ((this._marker = this._createRole(e)),
              this._marker.on("click", this._onClick, this),
              this._map
      ...
      _createRole: function (t) {
        return L.role(t, this.options);
      },
      _onClick: function () {
        this._fireCreatedEvent(),
          this.disable(),
      ...
      _fireCreatedEvent: function () {
        const t = this._createRole(this._mouseMarker.getLatLng());
        L.Draw.Feature.prototype._fireCreatedEvent.call(this, t);
      },
    })),
    (L.Draw.MayBienAp = L.Draw.Feature.extend({
      statics: { TYPE: "mayBienAp" },
      options: {
        icon: new L.Icon.Default(),
        repeatMode: !1,
        zIndexOffset: 2e3,
      ...
      initialize: function (t, e) {
        (this.type = L.Draw.MayBienAp.TYPE),
          (this._initialLabelText =
            L.drawLocal.draw.handlers.mayBienAp.tooltip.start),
          L.Draw.Feature.prototype.initialize.call(this, t, e);
      },
      addHooks: function () {
        L.Draw.Feature.prototype.addHooks.call(this),
      ...
      _onMouseMove: function (t) {
        ...
        this._tooltip.updatePosition(e),
          ...
          this._marker
            ? ((e = this._mouseMarker.getLatLng()),
              this._map.removeLayer(this._marker),
              (this._marker = this._createMayBienAp(e)),
              this._map.addLayer(this._marker))
            : ((this._marker = this._createMayBienAp(e)),
              this._marker.on("click", this._onClick, this),
              this._map
      ...
      _createMayBienAp: function (t) {
        return L.mayBienAp(t, this.options);
      },
      _onClick: function () {
        this._fireCreatedEvent(),
          this.disable(),
      ...
      _fireCreatedEvent: function () {
        var t = this._createMayBienAp(this._mouseMarker.getLatLng());
        L.Draw.Feature.prototype._fireCreatedEvent.call(this, t);
      },
    })),
    (L.Edit = L.Edit || {}),
    ...
    (L.Edit.ThanhCai = L.Edit.SimpleShape.extend({
      initialize: function (t, e) {
        L.Edit.SimpleShape.prototype.initialize.call(this, t, e);
      },
      _createMoveMarker: function () {
      ...
      _createResizeMarker: function () {
        this._resizeMarkers = [];
        const t = this._shape.getLatLngs(),
          e = t[0];
        this._resizeMarkers.push(
          this._createMarker(e, this.options.resizeIcon)
        );
      ...
      _createRotateMarker: function () {
        const t = this._shape.getRotateMarker();
        this._rotateMarker = this._createMarker(t, this.options.moveIcon);
      },
      _move: function (t) {
        this._shape.move(t),
          this._updateMarkers(),
      ...
      _resize: function (t) {
        this._shape.resize(t),
          this._updateMarkers(),
          this._map.fire(L.Draw.Event.EDITRESIZE, { layer: this._shape });
      },
      _rotate: function (t) {
        this._shape.rotate(t), this._updateMarkers();
      },
    ...
    (L.Edit.Role = L.Edit.SimpleShape.extend({
      ...
      _createResizeMarker: function () {
        ...
        (this._resizeMarkers = []),
          this._resizeMarkers.push(
            this._createMarker(t[0], this.options.resizeIcon)
          );
      },
      _createRotateMarker: function () {
        const t = this._shape.getRotateMarker();
    ...
    (function (e) {
      ...
      if ("function" == typeof define && define.amd) define(["leaflet"], e);
        ...
        (i = require("leaflet")), (module.exports = e(i));
      else {
        if (void 0 === t.L) throw "Leaflet must be loaded first";
        e(t.L);
      }
    })(function (t) {
      "use strict";
      return (
        (t.Polyline._flat =
          ...
          function (e) {
            return (
              !t.Util.isArray(e[0]) ||
              ("object" != typeof e[0][0] && void 0 !== e[0][0])
            );
          }),
        (t.GeometryUtil = t.extend(t.GeometryUtil || {}, {
          ...
          distanceSegment: function (e, i, o, n) {
            var s = e.latLngToLayerPoint(i),
              a = e.latLngToLayerPoint(o),
              r = e.latLngToLayerPoint(n);
            return t.LineUtil.pointToSegmentDistance(s, a, r);
          },
          ...
          belongsSegment: function (t, e, i, o) {
            o = void 0 === o ? 0.2 : o;
            var n = e.distanceTo(i);
            return (e.distanceTo(t) + t.distanceTo(i) - n) / n < o;
          },
          length: function (e) {
            var i = t.GeometryUtil.accumulatedLengths(e);
            return i.length > 0 ? i[i.length - 1] : 0;
          },
          accumulatedLengths: function (t) {
            if (
              ("function" == typeof t.getLatLngs && (t = t.getLatLngs()),
              0 === t.length)
            )
              return [];
            for (var e = 0, i = [0], o = 0, n = t.length - 1; o < n; o++)
              (e += t[o].distanceTo(t[o + 1])), i.push(e);
            return i;
          },
          closestOnSegment: function (e, i, o, n) {
            var s = e.getMaxZoom();
            s === 1 / 0 && (s = e.getZoom());
            var a = e.project(i, s),
              r = e.project(o, s),
              h = e.project(n, s),
              l = t.LineUtil.closestPointOnSegment(a, r, h);
            return e.unproject(l, s);
          },
          closest: function (e, i, o, n) {
            var s,
              a,
              r,
              h,
            ...
            if (i instanceof Array) {
              if (i[0] instanceof Array && "number" != typeof i[0][0]) {
                for (a = 0; a < i.length; a++)
                  (l = t.GeometryUtil.closest(e, i[a], o, n)) &&
                    l.distance < c &&
                    ((c = l.distance), (d = l));
                return d;
              }
              if (
                !(
                  i[0] instanceof t.LatLng ||
                  "number" == typeof i[0][0] ||
                  "number" == typeof i[0].lat
                )
              )
                return d;
              i = t.polyline(i);
            }
            if (!(i instanceof t.Polyline)) return d;
            if (
              ((s = JSON.parse(JSON.stringify(i.getLatLngs().slice(0)))),
              i instanceof t.Polygon)
            ) {
              var _ = function (e) {
                if (t.Polyline._flat(e)) e.push(e[0]);
                else for (var i = 0; i < e.length; i++) _(e[i]);
              };
              _(s);
            }
            if (t.Polyline._flat(s)) {
              if (n) {
                for (a = 0, r = s.length; a < r; a++) {
                  var u = s[a];
                  (h = t.GeometryUtil.distance(e, o, u)),
                    h < c && ((c = h), (d = u), (d.distance = h));
                }
                return d;
              }
              for (a = 0, r = s.length; a < r - 1; a++) {
                var p = s[a],
                  L = s[a + 1];
                (h = t.GeometryUtil.distanceSegment(e, o, p, L)),
                  h <= c &&
                    ((c = h),
                    (d = t.GeometryUtil.closestOnSegment(e, o, p, L)),
                    (d.distance = h));
              }
              return d;
            }
            for (a = 0; a < s.length; a++)
              (l = t.GeometryUtil.closest(e, s[a], o, n)),
                l.distance < c && ((c = l.distance), (d = l));
            return d;
          },
          closestLayer: function (e, i, o) {
            for (
              var n = 1 / 0, s = null, a = null, r = 1 / 0, h = 0, l = i.length;
              h < l;
              h++
            ) {
              var c = i[h];
              if (c instanceof t.LayerGroup) {
                var d = t.GeometryUtil.closestLayer(e, c.getLayers(), o);
                d.distance < n && ((n = d.distance), (s = d));
              } else
                "function" == typeof c.getLatLng
                  ? ((a = c.getLatLng()),
                    (r = t.GeometryUtil.distance(e, o, a)))
                  : (a = t.GeometryUtil.closest(e, c, o)) && (r = a.distance),
                  r < n &&
                    ((n = r), (s = { layer: c, latlng: a, distance: r }));
            }
            return s;
          },
          nClosestLayers: function (e, i, o, n) {
            if ((n = "number" == typeof n ? n : i.length) < 1 || i.length < 1)
              return null;
            for (var s, a, r = [], h = 0, l = i.length; h < l; h++) {
              var c = i[h];
              if (c instanceof t.LayerGroup) {
                var d = t.GeometryUtil.closestLayer(e, c.getLayers(), o);
                r.push(d);
                ...
                "function" == typeof c.getLatLng
                  ? ((a = c.getLatLng()),
                    (s = t.GeometryUtil.distance(e, o, a)))
                  : (a = t.GeometryUtil.closest(e, c, o)) && (s = a.distance),
                  r.push({ layer: c, latlng: a, distance: s });
            }
            return (
              r.sort(function (t, e) {
          ...
          layersWithin: function (e, i, o, n) {
            n = "number" == typeof n ? n : 1 / 0;
            for (var s = [], a = null, r = 0, h = 0, l = i.length; h < l; h++) {
              var c = i[h];
              "function" == typeof c.getLatLng
                ? ((a = c.getLatLng()), (r = t.GeometryUtil.distance(e, o, a)))
                : (a = t.GeometryUtil.closest(e, c, o)) && (r = a.distance),
                a && r < n && s.push({ layer: c, latlng: a, distance: r });
            }
            return s.sort(function (t, e) {
              return t.distance - e.distance;
            });
          },
          closestLayerSnap: function (e, i, o, n, s) {
            (n = "number" == typeof n ? n : 1 / 0),
              (s = "boolean" != typeof s || s);
            var a = t.GeometryUtil.closestLayer(e, i, o);
            if (!a || a.distance > n) return null;
            if (s && "function" == typeof a.layer.getLatLngs) {
              var r = t.GeometryUtil.closest(e, a.layer, a.latlng, !0);
              r.distance < n &&
                ((a.latlng = r),
                (a.distance = t.GeometryUtil.distance(e, r, o)));
            }
            return a;
          },
          interpolateOnPointSegment: function (e, i, o) {
            return t.point(e.x * (1 - o) + o * i.x, e.y * (1 - o) + o * i.y);
          },
          interpolateOnLine: function (e, i, o) {
            i = i instanceof t.Polyline ? i.getLatLngs() : i;
            var n = i.length;
            if (n < 2) return null;
            if (0 === (o = Math.max(Math.min(o, 1), 0)))
              return {
                latLng: i[0] instanceof t.LatLng ? i[0] : t.latLng(i[0]),
                predecessor: -1,
              };
            if (1 == o)
              return {
                latLng:
                  i[i.length - 1] instanceof t.LatLng
                    ? i[i.length - 1]
                    : t.latLng(i[i.length - 1]),
                predecessor: i.length - 2,
              };
            var s = e.getMaxZoom();
            s === 1 / 0 && (s = e.getZoom());
            for (var a = [], r = 0, h = 0; h < n; h++)
              (a[h] = e.project(i[h], s)),
                h > 0 && (r += a[h - 1].distanceTo(a[h]));
            for (var l = r * o, c = 0, d = 0, h = 0; d < l; h++) {
              var _ = a[h],
                u = a[h + 1];
              (c = d), (d += _.distanceTo(u));
            }
            if (void 0 == _ && void 0 == u)
              var _ = a[0],
                u = a[1],
                h = 1;
            var p = d - c != 0 ? (l - c) / (d - c) : 0,
              L = t.GeometryUtil.interpolateOnPointSegment(_, u, p);
            return { latLng: e.unproject(L, s), predecessor: h - 1 };
          },
          locateOnLine: function (e, i, o) {
            var n = i.getLatLngs();
            if (o.equals(n[0])) return 0;
            if (o.equals(n[n.length - 1])) return 1;
            for (
              var s = t.GeometryUtil.closest(e, i, o, !1),
                a = t.GeometryUtil.accumulatedLengths(n),
                r = a[a.length - 1],
                h = 0,
                l = !1,
                c = 0,
              ...
              var _ = n[c],
                u = n[c + 1];
              if (((h = a[c]), t.GeometryUtil.belongsSegment(s, _, u, 0.001))) {
                (h += _.distanceTo(s)), (l = !0);
                break;
              }
            }
            if (!l)
          ...
          extract: function (e, i, o, n) {
            if (o > n)
              return t.GeometryUtil.extract(
                e,
                t.GeometryUtil.reverse(i),
                1 - o,
                1 - n
              );
            ...
            var s = i.getLatLngs(),
              a = t.GeometryUtil.interpolateOnLine(e, i, o),
              r = t.GeometryUtil.interpolateOnLine(e, i, n);
            if (o == n) {
              return [t.GeometryUtil.interpolateOnLine(e, i, n).latLng];
            }
            -1 == a.predecessor && (a.predecessor = 0),
              -1 == r.predecessor && (r.predecessor = 0);
            var h = s.slice(a.predecessor + 1, r.predecessor + 1);
            return h.unshift(a.latLng), h.push(r.latLng), h;
          },
          isBefore: function (t, e) {
            if (!e) return !1;
            var i = t.getLatLngs(),
              o = e.getLatLngs();
            return i[i.length - 1].equals(o[0]);
          },
          isAfter: function (t, e) {
            if (!e) return !1;
            var i = t.getLatLngs(),
              o = e.getLatLngs();
            return i[0].equals(o[o.length - 1]);
          },
          startsAtExtremity: function (t, e) {
            if (!e) return !1;
            var i = t.getLatLngs(),
              o = e.getLatLngs(),
              n = i[0];
            return n.equals(o[0]) || n.equals(o[o.length - 1]);
          },
          computeAngle: function (t, e) {
            return (180 * Math.atan2(e.y - t.y, e.x - t.x)) / Math.PI;
          },
          computeSlope: function (t, e) {
            var i = (e.y - t.y) / (e.x - t.x);
            return { a: i, b: t.y - i * t.x };
          },
          rotatePoint: function (e, i, o, n) {
            var s = e.getMaxZoom();
            s === 1 / 0 && (s = e.getZoom());
            var a = (o * Math.PI) / 180,
              r = e.project(i, s),
              h = e.project(n, s),
              l = Math.cos(a) * (r.x - h.x) - Math.sin(a) * (r.y - h.y) + h.x,
              c = Math.sin(a) * (r.x - h.x) + Math.cos(a) * (r.y - h.y) + h.y;
            return e.unproject(new t.Point(l, c), s);
          },
          bearing: function (t, e) {
            var i = Math.PI / 180,
              o = t.lat * i,
              n = e.lat * i,
              s = t.lng * i,
              a = e.lng * i,
              r = Math.sin(a - s) * Math.cos(n),
              h =
                Math.cos(o) * Math.sin(n) -
                Math.sin(o) * Math.cos(n) * Math.cos(a - s),
              l = ((180 * Math.atan2(r, h)) / Math.PI + 360) % 360;
            return l >= 180 ? l - 360 : l;
          },
          destination: function (e, i, o) {
            i = (i + 360) % 360;
            var n = Math.PI / 180,
              s = 180 / Math.PI,
              a = 6378137,
              ...
              _ = Math.cos(o / a),
              u = Math.sin(o / a),
              p = Math.asin(c * _ + d * u * Math.cos(l)),
              L = r + Math.atan2(Math.sin(l) * u * d, _ - c * Math.sin(p));
            return (
              (L *= s),
              (L = L > 180 ? L - 360 : L < -180 ? L + 360 : L),
              t.latLng([p * s, L])
            );
          },
          angle: function (t, e, i) {
            var o = t.latLngToContainerPoint(e),
              n = t.latLngToContainerPoint(i),
              s = (180 * Math.atan2(n.y - o.y, n.x - o.x)) / Math.PI + 90;
            return (s += s < 0 ? 360 : 0);
          },
          destinationOnSegment: function (e, i, o, n) {
            var s = t.GeometryUtil.angle(e, i, o),
              a = t.GeometryUtil.destination(i, s, n);
            return t.GeometryUtil.closestOnSegment(e, a, i, o);
          },
    ...
    (L.DrawToolbar = L.Toolbar.extend({
      statics: { TYPE: "draw" },
      options: { role: {}, thanhCai: {}, mayBienAp: {}, duongDay: {} },
      initialize: function (t) {
    ...
    (L.EditToolbar.Edit = L.Handler.extend({
      ...
      save: function () {
        ...
        this._featureGroup.eachLayer(function (e) {
          if (e.edited) {
            var i = e.options;
            e instanceof L.Role
              ? ((i.original.chieuDai = i.chieuDai),
                (i.original.chieuRong = i.chieuRong),
                (i.original.gocXoay = i.gocXoay))
              : e instanceof L.ThanhCai
              ? ((i.original.chieuDai = i.chieuDai),
                (i.original.gocXoay = i.gocXoay))
              : e instanceof L.MayBienAp &&
                ((i.original.chieuDai = i.chieuDai),
                (i.original.gocXoay = i.gocXoay)),
              t.addLayer(e),
              (e.edited = !1);
      ...
      _backupLayer: function (t) {
        const e = L.Util.stamp(t);
        if (!this._uneditedLayerProps[e]) {
          const i = t.options;
          if (t instanceof L.Role)
          ...
          else if (t instanceof L.ThanhCai)
            this._uneditedLayerProps[e] = {
              latlngs: L.LatLngUtil.cloneLatLngs(t.getLatLngs()),
              options: { chieuDai: i.chieuDai, gocXoay: i.gocXoay },
            };
          else if (t instanceof L.MayBienAp) {
            const o = t.getLatLng();
            this._uneditedLayerProps[e] = {
      ...
      _revertLayer: function (t) {
        ...
        (t.edited = !1),
          this._uneditedLayerProps.hasOwnProperty(e) &&
            (L.setOptions(t, this._uneditedLayerProps[e].options),
            t instanceof L.Role || t instanceof L.ThanhCai
              ? t.setLatLngs(this._uneditedLayerProps[e].latlngs)
              : t instanceof L.MayBienAp &&

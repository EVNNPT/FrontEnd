...
L.Snap.processGuide = function (latlng, marker, guide, snaplist, buffer) {
  ...
  // Search snaplist around mouse
    ...
    snaplist = snaplist.concat(
      nearlayers.filter(function (layer) {
...
L.Snap.findClosestLayerSnap = function (
  map,
  layers,
  latlng,
  tolerance,
  ...
  for (var c = 0; c < closest.length; c++) {
    ...
    if (layerInfo.distance < tolerance) {
      ...
      if (layerInfo.layer.hasOwnProperty("_latlng")) {
        ...
        !layerInfo.layer.hasOwnProperty("_gridlineGroup") &&
        !layerInfo.layer.hasOwnProperty("_guidelineGroup")
      ) {
  ...
  if (pointsWithinTolerance.length > 0) {
    ...
    if (withVertices && typeof shapeInfo.layer.getLatLngs == "function") {
      var vertexLatLng = L.GeometryUtil.closest(
        map,
        shapeInfo.layer,
        shapeInfo.latlng,
        true
    ...
    for (var i = 0; i < withinTolerance.length; i++) {
      if (withinTolerance[i].layer._gridlineGroup != guideType) {
        intInfo = L.Snap.findGuideIntersection("guide", map, latlng, [
          guideInfo,
          withinTolerance[i],
        ]);
    ...
    if (withinTolerance.length == 2) {
      intInfo = L.Snap.findGuideIntersection(
        "grid",
        map,
        latlng,
        withinTolerance
...
L.Snap.snapMarker = function (e, guides, map, options, buffer) {
  ...
  var closest = L.Snap.findClosestLayerSnap(
    map,
    snaplist,
    latlng,
    options.snapDistance,
    options.snapVertices
  );
...
L.Handler.MarkerSnap = L.Handler.extend({
  ...
  initialize: function (map, marker, options) {
    ...
    function computeBuffer() {
      this._buffer =
        map.layerPointToLatLng(new L.Point(0, 0)).lat -
  ...
  addGuideLayer: function (layer) {
    for (var i = 0, n = this._guides.length; i < n; i++)
      if (L.stamp(layer) == L.stamp(this._guides[i])) return;
    this._guides.push(layer);
  },

  _snapMarker: function (e) {
    var closest = L.Snap.snapMarker(
      e,
      this._guides,
      this._map,
      this.options,
    ...
    if (
      ...
      var snapTouchPoint = this._map.project(
        closest.latlng,
        this._map.getZoom()
      );
...
L.Handler.PolylineSnap = L.Edit.Poly.extend({
  ...
  _createMoveMarker: function (latlng, icon) {
    var marker = L.Edit.Poly.prototype._createMoveMarker.call(
      this,
      latlng,
      icon
    );
  ...
  _initHandlers: function () {
    ...
    for (var i = 0; i < this.latlngs.length; i++) {
      this._verticesHandlers.push(
        new L.Edit.PolyVerticesEditSnap(
          this._poly,
          this.latlngs[i],
          this.options
        )
...
L.Edit.PolyVerticesEditSnap = L.Edit.PolyVerticesEdit.extend({
  _createMarker: function (latlng, index) {
    var marker = L.Edit.PolyVerticesEdit.prototype._createMarker.call(
      this,
      latlng,
      index
    );
...
L.Handler.RectangleSnap = L.Edit.Rectangle.extend({
  ...
  _createMarker: function (latlng, icon) {
    var marker = L.Edit.Rectangle.prototype._createMarker.call(
      this,
      latlng,
      icon
    );
  ...
  _enableLayerEdit: function (e) {
    ...
    if (!layer.snapediting) {
      if (layer.hasOwnProperty("_mRadius")) {
        ...
        layer.editing = layer.snapediting = new L.Handler.CircleSnap(
          layer._map,
          layer,
          this.snapOptions
        );
      } else if (layer.getLatLng) {
        layer.snapediting = new L.Handler.MarkerSnap(
          layer._map,
          layer,
          this.snapOptions
        );
        ...
        if (layer.editing) {
          if (layer.editing.hasOwnProperty("_shape")) {
            ...
            if (layer.editing._shape instanceof L.Rectangle) {
              delete layer.editing;
              layer.editing = layer.snapediting = new L.Handler.RectangleSnap(
                layer._map,
                layer,
                this.snapOptions
              );
              ...
              delete layer.editing;
              layer.editing = layer.snapediting =
                new L.Handler.FeatureGroupSnap(
                  layer._map,
                  layer,
                  this.snapOptions
                );
            ...
            delete layer.editing;
            layer.editing = layer.snapediting = new L.Handler.PolylineSnap(
              layer._map,
              layer,
              this.snapOptions
            );
...
L.Draw.Feature.SnapMixin = {
  ...
  _snap_on_click: function (e) {
    ...
    if (this._mouseDownOrigin) {
      ...
      if (e.originalEvent) {
        var oeOrigin = this._map.unproject(
  ...
  _manuallyCorrectClick: function (originalLatLng) {
    ...
    var buffer = 0;
    if (
      this.hasOwnProperty("_snapper") &&
      this._snapper.hasOwnProperty("_buffer")
    ) {
      buffer = this._snapper._buffer;
    }

    return L.Snap.snapMarker(
      ex,

(function () {
  L.Handler.MarkerSnap = L.Handler.extend({
      ...
      function isDifferentLayer(layer) {
        if (layer.getLatLng) {
          ...
          if (layer.editing && layer.editing._enabled) {
            var points =
              layer.editing._verticesHandlers[0]._markerGroup.getLayers();
            for (var i = 0, n = points.length; i < n; i++) {
              if (L.stamp(points[i]) === L.stamp(marker)) {
                return false;
              }
            }
          }
    ...
    _findClosestLayerSnap: function (
      map,
      layers,
      latlng,
      tolerance,
  ...
  L.Draw.ThanhCai.include(L.Draw.Feature.SnapMixin);
  L.Draw.ThanhCai.addInitHook("_snap_initialize");
